...
const quartos = [
  { id: "q5", nome: "Quarto Conforto", cap: "Até 4 pessoas", img: "/assets/Quarto5/1.jpeg", tags: ["Cama Casal", "2 Solteiros", "Ar-cond.", "Wi-Fi"], msg: "Olá! Tenho interesse no Quarto Conforto. Pode verificar disponibilidade?" },
  { id: "q6", nome: "Quarto Superior", cap: "Até 4 pessoas", img: "/assets/Quarto6/1.jpeg", tags: ["Cama Casal", "2 Solteiros", "Frigobar", "Wi-Fi"], msg: "Olá! Tenho interesse no Quarto Superior. Pode verificar disponibilidade?" },
  { id: "q7", nome: "Quarto Casal", cap: "Até 3 pessoas", img: "/assets/Quarto7/1.jpeg", tags: ["Cama Casal", "Ar-cond.", "Wi-Fi", "Privativo"], msg: "Olá! Tenho interesse no Quarto Casal. Pode verificar disponibilidade?" },
]
...
const depoimentos = [
  ...
  { texto: "Atendimento impecável desde a reserva até o checkout. Café da manhã farto e quarto muito confortável.", nome: "Ana Paula F.", local: "Google Reviews" },
  { texto: "Passei 3 noites a trabalho e foi excelente. Wi-Fi estável, silencioso e localização perfeita no centro.", nome: "Fábio L.", local: "Google Reviews" },
]
...
export default function Home() {
  ...
  useEffect(() => {
    const t = setInterval(() => setDepIdx(i => (i + 1) % depoimentos.length), 5000)
  ...
  return (
    <>
      ...
      {/* ── HERO ─────────────────────────────────── */}
      <section style={{ position: "relative", minHeight: "100vh", overflow: "hidden", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
        {/* Background */}
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, rgba(0,0,0,0.42) 0%, rgba(0,0,0,0.25) 45%, rgba(0,0,0,0.72) 100%)" }} />
        </div>

        {/* Conteúdo central */}
        <div style={{ flex: 1, display: "flex", alignItems: "center", position: "relative", zIndex: 1 }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "120px 2rem 2rem", width: "100%", textAlign: "center" }}>

            {/* Eyebrow */}
            <div className="animate-fade-up" style={{ animationDelay: "0.1s", opacity: 0, marginBottom: "1.5rem" }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", background: "rgba(255,255,255,0.1)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.18)", borderRadius: "100px", padding: "0.35rem 1.1rem" }}>
                <span style={{ color: "#F5C842", fontSize: "0.8rem", letterSpacing: "0.05em" }}>★★★★½</span>
                <span style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "0.75rem", color: "rgba(255,255,255,0.88)", fontWeight: 600, letterSpacing: "0.06em" }}>4,5 · 60+ avaliações no Google</span>
              </span>
            </div>

            {/* Headline */}
            <div className="animate-fade-up" style={{ animationDelay: "0.2s", opacity: 0 }}>
              <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(3rem, 7vw, 6rem)", fontWeight: 400, lineHeight: 1.05, color: "white", marginBottom: "1rem", textShadow: "0 2px 24px rgba(0,0,0,0.35)" }}>
                Conforto e acolhimento<br />
                <em style={{ fontStyle: "italic", opacity: 0.9 }}>no coração do Oeste Catarinense</em>
              </h1>
            </div>

            {/* Sub */}
            <div className="animate-fade-up" style={{ animationDelay: "0.32s", opacity: 0 }}>
              <p style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "1.05rem", fontWeight: 300, lineHeight: 1.8, color: "rgba(255,255,255,0.78)", maxWidth: "520px", margin: "0 auto 2.5rem" }}>
        ...
        {/* Booking Widget — âncora na base do hero */}
        <div style={{ position: "relative", zIndex: 2 }}>
          <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 2rem" }}>
            <div className="animate-fade-up" style={{ animationDelay: "0.44s", opacity: 0 }}>
              <BookingWidget />
            </div>
          </div>

          {/* Stats bar */}
          <div style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(12px)", borderTop: "1px solid rgba(255,255,255,0.08)", marginTop: "0" }}>
            <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 2rem", display: "flex", justifyContent: "space-around", flexWrap: "wrap" as const }}>
              {stats.map(s => (
                <div key={s.label} style={{ padding: "1rem 1rem", textAlign: "center" }}>
                  <div className="stat-num" style={{ color: "white", fontSize: "clamp(1.1rem, 2vw, 1.5rem)" }}>{s.num}</div>
                  <div style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "rgba(255,255,255,0.5)", marginTop: "0.2rem" }}>{s.label}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 2rem", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "5rem", alignItems: "center" }}>
          <ScrollReveal className="from-left">
            <div style={{ position: "relative" }}>
              ...
              {/* Badge */}
              <div style={{ position: "absolute", bottom: "-1.5rem", right: "-1rem", background: "var(--wine)", color: "white", borderRadius: "10px", padding: "1.25rem 1.75rem", boxShadow: "0 8px 30px rgba(107,91,62,0.35)" }}>
                <div style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "1.8rem", lineHeight: 1 }}>4,5 ★</div>
                ...
                <div className="card" style={{ cursor: "default" }}>
                  ...
                  <div style={{ padding: "1.5rem" }}>
                    ...
                    <div style={{ display: "flex", gap: "0.75rem", alignItems: "center" }}>
                      <a href={WA_URL(q.msg)} target="_blank" rel="noopener noreferrer" className="btn-wine" style={{ fontSize: "0.75rem", padding: "0.65rem 1.25rem", flex: 1, justifyContent: "center" }}>
                        Reservar
                      </a>
                      <Link href={`/quartos/${q.id}`} style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "0.8rem", fontWeight: 600, color: "var(--accent)", textDecoration: "none", whiteSpace: "nowrap" as const }}>
                        Ver detalhes →
                      </Link>
                    </div>
                  </div>
      ...
      {/* ── GALERIA STRIP ────────────────────────── */}
      <section style={{ background: "var(--cream-alt)", padding: "0", overflow: "hidden", position: "relative" }}>
        <Link href="/galeria" style={{ display: "block", textDecoration: "none" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", height: "240px" }}>
            {["/assets/Corredores/2.jpeg", "/assets/Quarto5/2.jpeg", "/assets/Corredores/6.jpeg", "/assets/Quarto6/2.jpeg", "/assets/Quarto7/2.jpeg"].map((src, i) => (
              <div key={i} className="img-zoom" style={{ position: "relative", height: "240px" }}>
                <Image src={src} alt="" fill style={{ objectFit: "cover" }} />
              </div>
            ))}
          </div>
          <div style={{ position: "absolute", inset: 0, background: "rgba(31,27,22,0.38)", display: "flex", alignItems: "center", justifyContent: "center", transition: "background 0.3s" }}>
            <span style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(1.4rem, 3vw, 2.2rem)", fontWeight: 400, color: "white", letterSpacing: "0.04em", textShadow: "0 2px 16px rgba(0,0,0,0.5)" }}>
      ...
      <section style={{ background: "var(--wine)", padding: "7rem 0", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", top: "2rem", left: "2rem", fontFamily: "Georgia, serif", fontSize: "20rem", lineHeight: 1, color: "rgba(255,255,255,0.04)", pointerEvents: "none", userSelect: "none" as const }}>&ldquo;</div>
        <div style={{ maxWidth: "800px", margin: "0 auto", padding: "0 2rem", textAlign: "center" }}>
          <ScrollReveal>
            <span style={{ display: "block", fontFamily: "Lato, Arial, sans-serif", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase" as const, color: "rgba(255,255,255,0.6)", marginBottom: "1rem" }}>Depoimentos</span>
            <h2 className="display" style={{ fontSize: "clamp(1.8rem, 4vw, 2.8rem)", color: "white", marginBottom: "3.5rem" }}>O que nossos hóspedes dizem</h2>
          </ScrollReveal>
          ...
          <div style={{ position: "relative", minHeight: "200px" }}>
            {depoimentos.map((d, i) => (
              <div key={d.nome} style={{
                ...
                opacity: depIdx === i ? 1 : 0,
                transform: depIdx === i ? "translateY(0)" : "translateY(12px)",
                transition: "opacity 0.6s ease, transform 0.6s ease",
                pointerEvents: depIdx === i ? "auto" : "none",
              }}>
                <div style={{ display: "flex", gap: "3px", justifyContent: "center", marginBottom: "1.5rem" }}>
                  {[...Array(5)].map((_,k) => <svg key={k} width="16" height="16" viewBox="0 0 24 24" fill="#F5C842" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>)}
                </div>
                <p style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontSize: "clamp(1.1rem, 2.5vw, 1.5rem)", fontWeight: 400, lineHeight: 1.65, color: "rgba(255,255,255,0.92)", fontStyle: "italic", marginBottom: "2rem" }}>
                  &ldquo;{d.texto}&rdquo;
                </p>
                <div>
                  <p style={{ fontFamily: "Lato, Arial, sans-serif", fontWeight: 700, color: "white", fontSize: "0.9rem" }}>{d.nome}</p>
                  <p style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "0.75rem", color: "rgba(255,255,255,0.45)", marginTop: "0.2rem" }}>{d.local}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: "flex", gap: "0.5rem", justifyContent: "center", marginTop: "3rem" }}>
            {depoimentos.map((_, i) => (
              <button key={i} onClick={() => setDepIdx(i)} style={{
                width: depIdx === i ? "24px" : "8px", height: "8px",
                borderRadius: "100px", border: "none", cursor: "pointer",
                background: depIdx === i ? "white" : "rgba(255,255,255,0.3)",
      ...
      {/* ── FAQ ──────────────────────────────────── */}
      <section style={{ background: "var(--cream-alt)", padding: "7rem 0" }}>
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 2rem", display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: "4rem", alignItems: "start" }}>
          <ScrollReveal>
            ...
            <h2 className="display" style={{ fontSize: "clamp(1.8rem, 4vw, 2.8rem)", color: "var(--text)", marginBottom: "1rem" }}>
              Perguntas<br />frequentes
            </h2>
            <p style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "0.9rem", color: "var(--text-muted)", lineHeight: 1.8, marginBottom: "2rem" }}>
              Ficou com alguma dúvida? Veja as perguntas mais comuns ou fale diretamente conosco.
            </p>
            <Link href="/contato" style={{ fontFamily: "Lato, Arial, sans-serif", fontSize: "0.82rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--accent)", textDecoration: "none", borderBottom: "2px solid var(--accent)", paddingBottom: "2px" }}>
              Falar com a gente →
            </Link>
          </ScrollReveal>
          <div>
